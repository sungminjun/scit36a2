...
//		System.out.println("File : " + savedFilename);
		...
		/*
		 * while (true) { serverFile = new File(uploadPath + "/" + savedFilename); //같은
		 * 이름의 파일이 없으면 나감. if (!serverFile.isFile()) break; //같은 이름의 파일이 있으면 이름 뒤에 long
		 * 타입의 시간정보를 덧붙임. filename = filename + new Date().getTime() ; }
		 */
